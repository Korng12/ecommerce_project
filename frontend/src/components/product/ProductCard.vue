<template>
  <router-link :to="{ name: 'productView', params: { productId: product.id } }" :class="product.stock <= 0 ? 'pointer-events-none' : ''">
    ...
    >
      ...
      <div v-if="product.stock <= 0" class="absolute inset-0 z-20 flex items-center justify-center bg-black/60 rounded-xl">
      ...
      <div class="relative overflow-hidden bg-gray-100 h-48">
        ...
        <div v-if="product.promotion" class="absolute top-3 left-3 bg-blue-600 text-white px-3 py-1.5 rounded-lg text-xs font-bold shadow-lg flex items-center gap-1.5">
          <svg class="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 20 20">
            <path d="M4.5 3a2.5 2.5 0 015 0v.006h.01a7 7 0 013.922 12.255.5.5 0 01-.714-.712A6 6 0 009.5 3.5h-.01V3a2.5 2.5 0 00-5 0v.5H4a2 2 0 00-2 2v11a2 2 0 002 2h12a2 2 0 002-2V5.5a2 2 0 00-2-2h-.5v-.5a2.5 2.5 0 00-5 0v.006h-.01A7 7 0 014.5 3z"/>
          </svg>
          ...
          <div class="flex items-center">
            <div class="flex gap-0.5">
              <span v-for="i in 5" :key="i" class="text-base">
                ...
                <span v-else-if="i - product.rating < 1" class="text-amber-400">⭐</span>
                <span v-else class="text-gray-300">★</span>
              </span>
            </div>
            <span class="ml-2 text-xs text-gray-600 font-medium">
              {{ product.rating }}/5 ({{ product.totalReviews }})
            </span>
          </div>
        </div>
