    ...
    <div class="max-w-7xl mx-auto px-6 lg:px-16 flex justify-between items-center h-16">
      ...
      <ul class="hidden md:flex items-center space-x-8 font-medium text-gray-700">
        ...
        <li class="relative group">
          <button class="flex items-center gap-1 hover:text-blue-600 transition">
            Categories
            <svg class="w-4 h-4 transform group-hover:rotate-180 transition-transform" fill="none" stroke="currentColor"
              viewBox="0 0 24 24">
        ...
        <li><router-link :to="{ name: 'contactUsView', path: '/contactUsView' }"
            class="hover:text-blue-600 transition">Contact</router-link></li>
      </ul>
      ...
      <div class="flex items-center gap-4 md:gap-6 relative">
        ...
        <div class="relative group">
          <button
            class="flex items-center gap-1 p-2 rounded-full hover:bg-gray-200 transition-transform duration-200 hover:scale-110">
    ...
    <transition name="fade-slide">
      <div v-if="searchOpen" class="w-full bg-white border-t border-gray-200 shadow-inner px-6 lg:px-16 py-3">
        ...
        <ul v-if="suggestions.length"
          class="max-w-7xl mx-auto mt-2 bg-white rounded-lg shadow-md overflow-hidden border border-gray-200">
          <li v-for="(item, i) in suggestions" :key="i" class="px-4 py-2 hover:bg-gray-100 cursor-pointer transition"
